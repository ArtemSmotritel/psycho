...
| 5 | Appointment lifecycle | Three states: **upcoming** (future), **active** (psychologist has manually started it), **past** (psychologist has manually ended it). Whiteboard accessible only during **active**. Notes/impressions available for any **past** appointment. Only one appointment can be active at a time per psychologist. |
...
| 10 | Recommendation reactions | Client can mark a recommendation as done/not done and leave a comment. Psychologist can see the comment and reply once (one level deep). No notifications for now. |
...
| 14 | Multi-role user role switching | One role is "active" at a time, stored in session. Dual-role users see a role selection screen after login. Role switcher is accessible inside the sidebar (not in main nav). Needs to be added to EDG-7 scope. |
...
| 16 | Client/psychologist disconnect | Only the **psychologist** can remove a client from their list. Historical data (appointments, notes, recommendations) is preserved and readable by both parties after disconnection. No new appointments can be created. Clients cannot disconnect unilaterally. |
...
| 27 | Impression submission rules | Multiple impressions can be submitted per appointment, each timestamped. No editing after submission. Progress timeline shows all entries chronologically. |
...
| Psychologist notes | Private per-appointment. Text, image, or audio. Visible to psychologist only. Can be created or edited on any past appointment. |
| Client impressions | Post-appointment reflections. Text, image, or audio. Multiple submissions per appointment allowed; each timestamped; no editing after submission. Visible to both client and psychologist (shown on psychologist's past appointment detail view). (Decisions 24, 27) |
...
| Email reminders | Triggers: (1) before upcoming appointments, (2) new recommendation created, (3) appointment deleted, (4) appointment rescheduled, (5) psychologist acts on a reschedule or cancellation request, (6) existing registered user receives a new invitation. Channel: email only. |
