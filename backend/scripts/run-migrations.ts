...
async function runMigrations() {
    try {
        ...
        const appliedMigrationsResult = await db`SELECT name FROM ${db(SCHEMA_MIGRATIONS_TABLE)};`
        console.log(appliedMigrationsResult)
        const appliedMigrations = new Set(appliedMigrationsResult.map((row: any) => row.name))
        ...
        const sortedMigrationFiles = migrationFiles.filter((file) => file.endsWith('.sql')).sort()

        if (sortedMigrationFiles.length === 0) {
            console.log('No SQL migration files found in the directory. No migrations to run.')
            return
        }
